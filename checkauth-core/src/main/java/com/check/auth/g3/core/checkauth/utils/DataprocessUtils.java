...
public class DataprocessUtils {
	public static List<AuthInstEntity> getAuthInst() throws IOException {
		...
		for (String string : lines) {
			if (StringUtils.isNotBlank(string)) {
				...
				authInstEntity.setContactPeroson(strs[14]);
	...
	public static List<AuthInstMainInfoEntity> getMainInstInfo() throws IOException {
		List<AuthInstMainInfoEntity> authInstMainInfoEntities = new ArrayList<AuthInstMainInfoEntity>();
		String path = "D:\\github\\CheckAuth\\checkauth-core\\src\\main\\resources\\data\\authorg.txt";
		List<String> lines = Files.readAllLines(Paths.get(path), Charset.forName("UTF-8"));
		for (String string : lines) {
			if (StringUtils.isNotBlank(string)) {
